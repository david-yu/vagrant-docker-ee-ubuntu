...
docker run --rm docker/dtr:${DTR_VERSION} backup --ucp-url https://${UCP_IPADDR} --existing-replica-id ${DTR_REPLICA_ID} --ucp-username docker --ucp-password ${UCP_PASSWORD} --ucp-insecure-tls > /tmp/backup.tar
